...
def plot_individual_errors_history():
    
    filename = "individual_errors_history.csv"
    if not os.path.exists(filename):
        print("Nessun file individual_errors_history.csv trovato.")
        return
    ...
    if not rows or len(rows) < 2:
        print("File individual_errors_history.csv vuoto o senza dati.")
        return
    ...
    plt.title('Evoluzione individual error per robot')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def main(csv_input_file, temp_txt_file, modified_traj_file, robot_index, threshold, mean, std):
    ...
    # Carica stato penalità/premio progressivo
    penalty_state = {}
    penalty_file = "robot_penalty_state.csv"
    if os.path.exists(penalty_file):
        with open(penalty_file, newline='') as file:
            reader = csv.reader(file)
            for row in reader:
                if len(row) == 3:
                    robot_id = int(row[0])
                    worst_count = int(row[1])
                    best_count = int(row[2])
                    penalty_state[robot_id] = {"worst": worst_count, "best": best_count}
   ...
   # Step 2: Parsing degli errori
    ...
    # Salva la storia degli errori individuali
    header_file = "individual_errors_history.csv"
    write_header = not os.path.exists(header_file)
    with open(header_file, mode="a", newline='') as file:
        writer = csv.writer(file)
        if write_header:
            writer.writerow(["timestamp"] + [f"robot_{robot_id}" for robot_id in sorted(robot_errors.keys())])
        row = [time.time()] + [robot_errors[robot_id] for robot_id in sorted(robot_errors.keys())]
        writer.writerow(row)


    if prev_robot_errors:
        # Penalizza in base agli errori della iterazione precedente
        base_info_value = 100.0
        min_info_value = 1e-10
        worst_factor = 0.00001
        ...
        # Aggiorna stato penalità/premio
        for robot_id in prev_robot_errors:
            if robot_id == worst_robot:
                penalty_state.setdefault(robot_id, {"worst": 0, "best": 0})
                penalty_state[robot_id]["worst"] += 1
            else:
                penalty_state.setdefault(robot_id, {"worst": 0, "best": 0})
                penalty_state[robot_id]["best"] += 1

        
        max_info_value = 1000  # Valore massimo consentito per la info matrix

        penalties = {}
        for robot_id, error in prev_robot_errors.items():
            norm_error = error / total_error if total_error > 0 else 0

            # Penalizzazione progressiva per il peggiore
            worst_factor = 0.98 ** penalty_state[robot_id]["worst"] if robot_id == worst_robot else 1.0

            # Premio progressivo per i migliori
            reward_factor = 1.15 ** penalty_state[robot_id]["best"] if robot_id != worst_robot else 1.0

            info_value = base_info_value * (1 - norm_error) * reward_factor
            if robot_id == worst_robot:
                info_value *= worst_factor

            # Limita il valore massimo e minimo
            info_value = max(min(info_value, max_info_value), min_info_value)
   ...
   #save a csv file with the results
    ...
    # Salva stato penalità/premio progressivo
    with open(penalty_file, mode="w", newline='') as file:
        writer = csv.writer(file)
        for robot_id, state in penalty_state.items():
            writer.writerow([robot_id, state["worst"], state["best"]])
...
atexit.register(plot_individual_errors_history) #grafico individual errors history
