...
def main(csv_input_file, temp_txt_file, output_file, robot_index, threshold, mean, std):
    ...
    output_file = "argos_ordered_traj_modified.g2o"  # Modifica col nome corretto

    try:
       result = subprocess.run([gnc_exec, output_file], check=True, text=True, capture_output=True)
...
# ✅ Ora `robot_errors` è un dizionario Python: {0: ..., 1: ..., ...}
    ...
    output_file = "output_optimized.g2o"

    try:
       result = subprocess.run([gnc_opt_exec, input_file, output_file], check=True, text=True, capture_output=True)
