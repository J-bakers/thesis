...
def main(csv_input_file, temp_txt_file, modified_traj_file, robot_index, threshold, mean, std):
    ...
    # Copy the CSV to a temporary file to avoid conflicts using shutil
    ...
    modified_traj_file = "argos_ordered_traj_modified.g2o"  # Modifica col nome corretto, output file is the PG where i modified the trajectory of one robot

    try:
        with open("worst_robot.txt", "r") as f:             #if 1st iteration prev_worst_robot=None , so we don't want to update info_matrix
            prev_worst_robot = int(f.read().strip())
    except Exception:
        prev_worst_robot = None        
            

 #new info matrix
    info_value = 0.01  # oppure qualsiasi altro valore che vuoi usare
    new_info_matrix = make_info_matrix(info_value)
    


 # Elabora file .g2o solo se prev_worst_robot is not None
    vertex_robot_map = build_vertex_robot_map("argos_ordered_traj_modified.g2o")

    if prev_worst_robot is not None:
            with open("argos_ordered_traj_modified.g2o", "r") as infile, open("argos_ordered_traj_modified_cov_modified.g2o", "w") as outfile:
                for line in infile:
                    if line.startswith("EDGE_SE3:QUAT"):
                        tokens = line.strip().split()
                        id1 = int(tokens[1])
                        ...
                        if robot1 == robot2 == prev_worst_robot:
                        ...
                        outfile.write(line)
            print(f"✅ argos_ordered_traj_modified_cov_modified.g2o'")
    else:
            print("⚠️ Nessun robot da modificare nella matrice informativa.")


 #individual errors
    gnc_exec = "/home/iacopo/optimizer/gnc_individual_error"

    if os.path.exists("argos_ordered_traj_modified_cov_modified.g2o"):            #if 1st iteration we calculate individual errors of argos_ordered_traj_modified.g2o otherwise we calculate individual errors of argos_ordered_traj_modified_cov_modified.g2o
        modified_traj_file = "argos_ordered_traj_modified_cov_modified.g2o"
    else:
...
# ✅ Ora `robot_errors` è un dizionario Python: {0: ..., 1: ..., ...}
    print("📊 Errori individuali:", robot_errors)
    
    
    # Trova il robot con l'errore massimo
    if robot_errors:
        worst_robot, max_error = max(robot_errors.items(), key=lambda x: x[1])
        print(f"🚨 Robot con errore massimo: {worst_robot} (errore = {max_error})")
        # Save worst_robot to file for next iteration
        with open("worst_robot.txt", "w") as f:
            f.write(str(worst_robot))
    else:
        print("⚠️ Nessun errore trovato nel dizionario.")
        worst_robot = None
        # Optionally clear the file
        open("worst_robot.txt", "w").close()
    
    
#optimization for saving the optimized output and global error
    gnc_opt_exec = "/home/iacopo/optimizer/gnc_optimizer"
    
    if os.path.exists("argos_ordered_traj_modified_cov_modified.g2o"):
        input_file = "argos_ordered_traj_modified_cov_modified.g2o"
    else:
        input_file = "argos_ordered_traj_modified.g2o"

    optimized_file = "output_optimized.g2o"


    try:
       result = subprocess.run([gnc_opt_exec, input_file, optimized_file], check=True, text=True, capture_output=True)
       ...
       print("❌ Eseguibile gnc_optimize non trovato.")
       
#grom g2o to tum format for evo
    g2o_to_tum('output_optimized.g2o', 'output_optimized.txt')
    g2o_to_tum('processed_posegraph.g2o', 'processed_posegraph.txt')
    
#evo api for ATE
    ground_truth = file_interface.read_tum_trajectory_file("processed_posegraph.txt")
    optimized_PG = file_interface.read_tum_trajectory_file("output_optimized.txt")
    
    ground_truth, optimized_PG = associate_trajectories(ground_truth, optimized_PG)
    
    ape_result = main_ape.ape(ground_truth, optimized_PG, pose_relation=PoseRelation.translation_part, align=True)
...
#saves the metrics from evo for plotting
    with open("ape_metrics.csv", mode="a", newline='') as file:
        writer = csv.writer(file)
    
        # Se vuoi aggiungere un timestamp o numero iterazione per tenere traccia
        writer.writerow(["--- Nuova Iterazione ---"])
    
        for key, value in ape_result.stats.items():
            writer.writerow([key, value])
    
        writer.writerow([])  # Riga vuota per separare visivamente le iterazioni
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interval", type=int, default=15, help="Ogni quanti secondi eseguire il processo")
