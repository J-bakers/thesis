...
def plot_results():
        ...
        # Assumiamo che baseline_means e penalized_means abbiano lo stesso numero di elementi
        ...
        plt.ylabel('Valore mean')
        plt.title('Valori mean per BASELINE e PENALIZED')
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.show()
...
def main(csv_input_file, temp_txt_file, modified_traj_file, robot_index, threshold, mean, std):
    ...
    # Step 3: Read previous worst robot and info value
    prev_worst_robot, prev_info_value = None, 0.01
    if os.path.exists("worst_robot.txt"):
        with open("worst_robot.txt", "r") as f:
            prev_worst_robot = int(f.read().strip())
    if os.path.exists("worst_robot_info_value.txt"):
        with open("worst_robot_info_value.txt", "r") as f:
            prev_info_value = float(f.read().strip())

    vertex_robot_map = build_vertex_robot_map(modified_traj_file)

    # Step 4: Modify info matrix if prev_worst_robot is known
    info_modified_file = modified_traj_file
    if prev_worst_robot is not None:
        next_info_value = max(prev_info_value / 2, 1e-20)
        new_info_matrix = make_info_matrix(next_info_value)
        ...
        with open(modified_traj_file, "r") as infile, open(info_modified_file, "w") as outfile:
            for line in infile:
                if line.startswith("EDGE_SE3:QUAT"):
                    ...
                    outfile.write(line)
        print(f"✅ Info matrix aggiornata per robot {prev_worst_robot} con valore {next_info_value}")
    else:
        print("⚠️ Nessun robot da modificare nella matrice informativa.")

    # Step 5: Calcolo degli errori individuali
    gnc_exec = "/home/iacopo/optimizer/gnc_individual_error"
    try:
        result = subprocess.run([gnc_exec, info_modified_file], check=True, text=True, capture_output=True)
    ...
    # Step 7: Salvataggio del robot peggiore
    if robot_errors:
        ...
        # Salva nuovo valore matrice info solo se robot è lo stesso
        if worst_robot == prev_worst_robot:
            with open("worst_robot_info_value.txt", "w") as f:
                f.write(str(next_info_value))
        else:
            with open("worst_robot_info_value.txt", "w") as f:
                f.write("0.01")
    else:
        ...
        open("worst_robot_info_value.txt", "w").close()
...
# Setup automatic plotting on exit
signal.signal(signal.SIGINT, lambda s, f: (plot_results(), sys.exit(0)))
