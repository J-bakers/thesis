...
def main(csv_input_file, temp_txt_file, modified_traj_file, robot_index, threshold, mean, std):
    ...
    try:
        with open("worst_robot.txt", "r") as f:             #if 1st iteration prev_worst_robot=None
...
#new info matrix
    robot_id_jump = 1000  # ogni robot ha ID nodi separati da almeno 1000
    new_info_matrix = [
        "1000", "0", "0", "0", "0", "0",
        "1000", "0", "0", "0", "0",
        "1000", "0", "0", "0",
        ...
        with open("argos_ordered_traj_modified.g2o", "r") as infile, open("argos_ordered_traj_modified_cov_modified.g2o", "w") as outfile:
            for line in infile:
                if line.startswith("EDGE_SE3:QUAT"):
                    ...
                    id1 = int(tokens[1])
                    id2 = int(tokens[2])
                    robot1 = id1 // robot_id_jump
                    robot2 = id2 // robot_id_jump
                    if robot1 == robot2 == prev_worst_robot:
                        new_line = " ".join(tokens[:9] + new_info_matrix) + "\n"
