...
def main(csv_input_file, temp_txt_file, modified_traj_file, robot_index, threshold, mean, std):
   ...
   # Step 3: Penalità inversa (più errore → meno fiducia)
    base_info_value = 1000.0
    min_info_value = 1e-3
    worst_factor = 0.1  # penalizza il peggiore di 10 volte in più

    total_error = sum(robot_errors.values()) if robot_errors else 1.0
    worst_robot = max(robot_errors, key=robot_errors.get) if robot_errors else None
    ...
    for robot_id, error in robot_errors.items():
        ...
        if robot_id == worst_robot:
            info_value *= worst_factor  # penalizza ancora di più il peggiore
        penalties[robot_id] = info_value

    print("Info matrix per robot (più errore → meno fiducia):", penalties)
